
import React from "react";
import { Link } from "react-router-dom";


const HeaderNavList=()=>{

    return(
        <div className="">
            <ul className="flex">
            <Link to={'/'}><li className="m-2 cursor-pointer hover:border-black">
                    <div className="px-6">
                        <img alt="" className="h-5" src="data:image/png;base64,"></img>
                    </div>
                    <span className="font-light text-xs">
                        Home 
                    </span>

                </li></Link>
                <li className="m-2">
                    <div className="px-6">
                        <img alt="" className="h-5" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTAC0EcFTdhYjnvc6Ad5nGf8MXzBEX0RLV8MQ&usqp=CAU"></img>
                    </div>
                    <span className="font-light text-xs">
                        My Network
                    </span>

                </li>

                <li className="m-2">
                    <div className="px-6">
                        <img alt="" className="h-5" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTo7uzJzS2dlaHYndCCGCtVwLZgoz7_ch-OSQ&usqp=CAU"></img>
                    </div>
                    <span className="font-light text-xs">
                        Jobs
                    </span>

                </li>

                <li className="m-2">
                    <div className="px-6">
                        <img alt="" className="h-5" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQb4XkK7bLyPeBwhO2qvpZUtfbpJFDgwjj5KpvaJk7ikQ&s"></img>
                    </div>
                    <span className="font-light text-xs">
                       Messaging
                    </span>

                </li>

                <li className="m-2">
                    <div className="px-6">
                        <img alt="" className="h-5" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSVrQV1D_bIFyKyYWqa-1P0J1nEU7n_ZeH7p3OOa9i4&s"></img>
                    </div>
                    <span className="font-light text-xs">
                    <Link to='/notify' className="cursor-pointer">  Notification </Link>
                    </span>

                </li>
            </ul>
        </div>
    )
}


export default HeaderNavList;